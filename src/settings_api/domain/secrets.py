...
keyring is two services away and it is built for this: encrypted at rest, never returned
by a read, audited. This service is the opposite of all three -- plaintext on disk,
returned in full to any token whose namespace grant permits it, and handed to six other
services on request. An API key written into ``search.default_model`` would be a
credential distributed to every consuming service at once, which is about the worst place
for one.

So a string value -- or any string inside a list value -- that looks like a credential is
refused with a 422 naming keyring as the right home. Not stripped, not masked, not
stored-and-flagged: refused, so the caller finds out immediately and puts it where it
goes.

This module is ported from user-api unchanged, corpora included, and that is deliberate.
Two services disagreeing about what a credential looks like would mean a value refused by
one and accepted by the other, and the one that accepted it would be the one that shares
it around.

## Why the heuristic is deliberately timid

**There is no override.** A false positive blocks a legitimate setting, and the person on
the other end has no way to insist -- they just cannot set the thing they wanted to set.
That asymmetry sets the tuning: a missed credential is a mistake the person can still
correct, and a wrongly refused value is a feature that does not work.
...
then it carves out the two shapes that keep coming up and pass every one of those: a git
hash and a hex colour.
...
``tests/unit/domain/test_secrets.py`` -- strings that must be accepted, and credential
shapes that must be refused. Change the heuristic and the corpora are what tell you
whether you made it better or just different. ``MUST_ACCEPT`` is the one allowed to grow,
and a change that shrinks it is a regression even if it catches more secrets.
"""
...
Two rules, and both were added in response to a string that should have been accepted.

**Case matters.** ``aizawa`` is a surname and ``AIza`` is a Google API key, and folding
the case would refuse somebody's friend.

**Position matters more.** Matched as a bare substring, ``sk-`` refuses "They are
risk-averse with money", and so does every "task-based", "desk-bound" and
"disk-encrypted" string anybody will ever write. The prefix must therefore begin a token
-- preceded by the start of the string or by something that is not alphanumeric -- and be
followed by enough credential-shaped characters to be one. There is no override, so a
false positive here is a setting somebody simply cannot set.
"""

MIN_PREFIXED_SUFFIX = 8
"""Characters a known prefix must be followed by before it reads as a credential.

Every real token of these kinds carries far more than eight. A bare ``sk-`` at the start
of a word is somebody's abbreviation.
"""
...
single most common long space-free run in prose. It was **unreachable**: every character
those patterns need is a colon, a dot or an at-sign, and this alphabet admits none of the
three, so a URL has already been let through one line earlier. It was removed rather than
left in as documentation, because the coverage gate is what found it and a branch no
input can take is a branch that stops being true without anybody noticing.
"""
...
def _is_high_entropy(run: str) -> bool:
    ...
    were added in response to a real string being refused, and each is a shape that is

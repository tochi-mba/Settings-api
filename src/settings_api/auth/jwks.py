...
anybody may fetch. Two consequences come with that. Both are inherited from keyring's own
design rather than chosen here, and both are terms of the arrangement rather than problems
...
expires. The alternative is a blocklist, and a blocklist is a call to keyring on every
request, which is the whole thing signed tokens exist to avoid.

**Nothing tells us an account is gone.** This service never asks keyring about an account,
so a keyring account deleted this morning leaves settings here that nobody has mentioned
it to. The levers are therefore local and both are deliberate: ``forget_settings`` is the
person's own and the database file is the operator's.

Note what this arrangement does *not* require, because it is the reason the service-facing
surface needs no change in keyring at all: a user token presented by a service is verified
against this same public document, with that service's audience. There is no token
exchange, no introspection endpoint, and no new keyring route.
...
these services are restarted together.
...
fails is :class:`~settings_api.domain.errors.KeyringUnreachableError` and becomes a 503
with a Retry-After. A fetch that succeeds and comes back without the key id is
:class:`~settings_api.domain.errors.AuthenticationError` and becomes a 401. Conflating
them tells a person to log in again because keyring was briefly down, and logging in again
...
It lives here rather than with the verifier because three modules refuse tokens -- an
unknown key id is refused before a signature is ever checked, and a service token is
refused before a user token is ever read -- and two spellings of "no" are two responses
somebody can tell apart while working out which forgery to try next.
"""
...
class JwksClient:
    ...
    async def healthy(self) -> tuple[bool, str | None]:
        ...
        broken", which are different things needing different people. user-api's liveness
        endpoint once returned 500 for exactly this reason.
    ...
    def _fresh_keys(self) -> jwt.PyJWKSet | None:
        ...
        the reason in :mod:`settings_api.core.clock`: a cache whose expiry can only be

...
Lifted from user-api, which lifted it from keyring. The reasoning is reproduced rather
than referenced, because the next person to edit this file should not have to read another
repository to know why it is shaped this way.
...
by construction rather than by convention -- which is what every cap and every revision
bump in this service is built on. The cost, and it belongs in the open: **a cancelled
...
Here that would mean a forgotten account keeping its settings rows: erasure that reports
success and erases nothing. That is why the connection is opened with
``isolation_level=None`` (this class issues its own ``BEGIN IMMEDIATE``) and why the
setting is read back and verified rather than assumed.

## The file mode is the whole defence

This database holds a person's own decisions in plaintext, unencrypted. The mode is not
defence in depth, it is the defence: 0600 on the file and on its ``-wal``/``-shm``
sidecars, which hold the same data the file does.

``synchronous = NORMAL`` rather than ``FULL``: a lost transaction here is a setting
somebody believes they changed, which is recoverable by changing it again, and WAL with
``NORMAL`` cannot corrupt the database -- it can only lose the last few commits on power
loss. ``PRAGMA secure_delete = ON`` overwrites freed pages rather than merely marking them
free; it is on because the freelist case it covers is real and it costs almost nothing,
not because it was observed to make a difference. :meth:`Database.checkpoint_truncate` is
the step that actually erases -- see :mod:`settings_api.settings.erasure`.
"""
...
class StorageError(RuntimeError):
    ...
    about a person or their decisions, and nothing above should be catching it. It means
...
def require_foreign_keys(connection: sqlite3.Connection) -> None:
    ...
    Read back rather than trusted. See the module docstring for how a
    ``PRAGMA foreign_keys`` can succeed and do nothing; the consequence here is an erasure
    that leaves every settings row behind while reporting that it deleted the account.
    """
...
def make_private(path: Path) -> None:
    ...
    0644. It is one ``open()`` wide, on a file with nothing in it yet, and closing it
    properly would mean pre-creating the file ourselves and hoping SQLite agreed with the
    result.
    """
...
class Database:
    ...
    :func:`settings_api.storage.migrator.migrate`.
    """

    def __init__(self, path: Path) -> None:
        self._path = path
        self._executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="settings-db")
        self._closed = False
    ...
    def _connect(self) -> sqlite3.Connection:
        # The mode applies only to directories this creates; an existing one is left
        # alone, because the configured path names a file and its parent may not be ours.
    ...
    async def transact(self, work: Callable[[sqlite3.Connection], T]) -> T:
        ...
        interleaved, because there is no other thread that could run one. The revision
        bump, the event append, the event trim and every cap in this service happen inside
        one of these, which is why "read the revision, then write" cannot go stale here.

        Anything ``work`` raises -- a domain error refusing the write included -- rolls
        the transaction back and propagates. That is what makes the document-shaped write
        all-or-nothing: one bad key raises, and every good key in the same document is
        rolled back with it.
        """
    ...
    async def checkpoint_truncate(self) -> None:
        ...
        **This is the step that actually erases.** A ``DELETE`` removes a row from the
        ...
        readable with ``grep``. ``FULL`` is not enough: it flushes the log into the
        database and leaves the log's pages where they are.

        ``VACUUM`` would also do it and is far more expensive -- it rewrites the entire
        database under a write lock.

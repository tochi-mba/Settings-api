...
datetime -- every retention calculation is against a UTC-aware one -- so a column that
returned a naive datetime would raise at the first comparison, in a code path far from the
store that produced it.

**Lexicographic order matches chronological order.** The event log's paging is by
sequence, but the sweeper compares ``set_at`` as text in SQL, and a comparison that sorted
wrongly would purge the wrong rows. That only works because the format is fixed width.

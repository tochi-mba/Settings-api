...
Hand-rolled rather than Alembic. What a migration tool buys is autogeneration from an ORM
and support for a dozen engines; there is no ORM here and there is one engine, so what
would be left is "run these files in order", which is this module.
...
One driver wrinkle worth knowing before editing this: ``executescript`` **commits any open
transaction before it runs**, so a migration cannot be wrapped in a ``BEGIN`` issued from
outside it. The ``BEGIN IMMEDIATE`` is therefore prepended to the script text, where
...
from settings_api.storage.times import to_column
...
def _apply(
    ...
    The rollback covers the script as well as the version row. A script that fails partway
    leaves its transaction *open* rather than unwinding it, so without this a broken
    migration would hold a write lock and leave half a schema behind.
    """
    try:

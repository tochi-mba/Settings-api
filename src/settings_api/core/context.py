...
Three facts are attached once, at the edge, and read wherever they are needed without
being threaded through every signature: the request id, the account the request was
authenticated as, and -- on the service-facing surface -- which service is calling.
Context variables are task-local, so two people's concurrent requests can never see each
other's identity, which is the mechanism the whole isolation story rests on and is tested
as such.
...
_request_id: ContextVar[str | None] = ContextVar("settings_request_id", default=None)
_account_id: ContextVar[str | None] = ContextVar("settings_account_id", default=None)
_service: ContextVar[str | None] = ContextVar("settings_calling_service", default=None)
...
def get_service() -> str | None:
    """Return the calling service, or ``None`` on the person-facing surface."""
    return _service.get()
...
def set_service(service: str) -> None:
    """Bind the calling service for the remainder of the current task.

    Set by the ``/v1/internal`` dependency, for the same reason and with the same
    lifetime as :func:`set_account_id`. Every log record from a service-facing request
    then says which service made it, which is what makes "who read this namespace"
...
@contextmanager
def bind_service(service: str) -> Iterator[str]:
    """Bind ``service`` for the duration of the block, restoring the previous value after."""

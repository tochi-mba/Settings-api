...
**Nothing in this module references another table.** ``accounts`` cascades to ``settings``;
``settings_events`` names no foreign key at all. An event has to outlive the setting it
describes, and the event that matters most is the one saying something was reset -- a
foreign key would delete that record along with the thing it recorded.
...
same failure the port refuses for the append itself. :data:`TRIM_TO_CAP` names the rows to
delete with a subselect instead, so there is no count that can go stale.
"""
...
from settings_api.events.log import Action, Event
...
APPENDED_COLUMNS = "account_id, at, action, namespace, key, revision, actor, service, detail"
...
TRIM_TO_CAP = (
    "DELETE FROM settings_events WHERE account_id = ? AND sequence <= ("
    "SELECT sequence FROM settings_events WHERE account_id = ? "
...
class SqlEventLog:
    ...
    # One parameter per column, matching the port. See
    # :class:`settings_api.events.log.EventLog` for why grouping them into an object would
    # only add a type to unpack again.
    def append_in(  # noqa: PLR0913
        ...
        *,
        ...
        connection.execute(
            f"INSERT INTO settings_events ({APPENDED_COLUMNS}) "  # noqa: S608
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                account_id,
                to_column(at),
                ...
                actor,
                service,
                # allow_nan=False for the reason in settings_api.domain.values: Python
                # renders NaN and Infinity as bare tokens that are not JSON, and a detail
                # column nothing else can parse is a log nothing else can read.
                None if detail is None else json.dumps(detail, allow_nan=False, sort_keys=True),
            ),
        )
    ...
    def delete_for_account_in(self, connection: sqlite3.Connection, *, account_id: str) -> int:
        deleted = connection.execute(
            "DELETE FROM settings_events WHERE account_id = ?", (account_id,)
        )
    ...
    async def read(
        ...
        rows = await self._db.fetch_all(
            f"SELECT {EVENT_COLUMNS} FROM settings_events WHERE account_id = ?{page}"  # noqa: S608
    ...
    async def count_for_account(self, account_id: str) -> int:
        return await self._db.count(
            "SELECT count(*) AS total FROM settings_events WHERE account_id = ?", (account_id,)
        )

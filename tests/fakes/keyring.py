...
``forge_hs256`` is the important part. PyJWT refuses to *sign* with a public key, which is
a guard on the signing side and not on the verifying side -- so the algorithm-confusion
...
from tests.fakes.clock import DECADE_SECONDS, EPOCH
...
def thumbprint(key: rsa.RSAPrivateKey = _SIGNING_KEY) -> str:
    ...
    Derived rather than random so it survives a restart -- which is also what makes "the
    kid changed" mean "the key was replaced" rather than "the process restarted".
    """
...
def mint(
    *,
    ...
    The default lifetime is a decade rather than keyring's own fifteen minutes, because
    several tests here move the clock months forward to exercise a retention window and
    would otherwise watch every good token expire underneath them. Tests that are *about*
    expiry pass their own ``ttl_seconds``.
...
def forge_hs256(*, account_id: str = "account-a", audience: str = "settings") -> str:
...
def forge_unsigned(*, account_id: str = "account-a", audience: str = "settings") -> str:
